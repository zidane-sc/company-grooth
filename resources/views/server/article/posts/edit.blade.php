                        ...
                        <form action="{{ route('posts.update', $data['post']->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') ?? $data['post']->title }}"  placeholder="Enter title">
                                            @error('title')
                                        ...
                                        <div class="form-group clearfix">
                                            ...
                                            <div class="icheck-primary d-inline ml-3">
                                              <input type="radio" id="drafted" name="status" value="DRAFTED" {{ (old('status') ?? $data['post']->status) == 'DRAFTED' ? 'checked' : '' }}>
                                              <label for="drafted" class="font-weight-normal">Drafted</label>
                                            </div>
                                            <div class="icheck-primary d-inline ml-3">
                                              <input type="radio" id="published" name="status" value="PUBLISHED" {{ (old('status') ?? $data['post']->status) == 'PUBLISHED' ? 'checked' : '' }}>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="select2-purple">
                                                <select class="select2" name="tags_id[]" id="tags_id"  multiple="multiple" data-placeholder="Select a Tags" data-dropdown-css-class="select2-purple" style="width: 100%;">
                                                    @empty($data['post']->tags))
                                                        @foreach ($data['tags'] as $tag)
                                                            <option value="{{ $tag->id }}">{{ $tag->name }}</option>
                                                        @endforeach
                                                    @else
                                                    @foreach ($data['tags'] as $tag)
                                                        @foreach ($data['post']->tags as $item)
                                                            @if ($item->id == $tag->id)
                                        ...
                                        <div class="form-group">
                                            ...
                                            <select class="form-control {{ $errors->first('category_id') ? 'is-invalid' : '' }}"  name="category_id" id="category_id">
                                                    ...
                                                    @foreach ($data['categories'] as $category)
                                                        <option value="{{ $category->id }}" {{ (old('category_id') ?? $data['post']->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                                    @endforeach
                                            </select>
                                        ...
                                        <div class="form-group">
                                            ...
                                            <div class="input-group mt-4">
                                                <div class="custom-file">
                                                    ...
                                                    <label class="custom-file-label @error('thumbnail') is-invalid @enderror" for="thumbnail">Change Thumbnail</label>
                                                </div>
                                            </div>
                                            @error('thumbnail')
                                ...
                                <div class="form-group mt-3">
                                    ...
                                    <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content"  rows="3" placeholder="Enter Desciption">{{ old('content') ?? $data['post']->content }}</textarea>
                            ...
                            <div class="card-footer">
                                ...
                                <a href="{{ route('posts.index') }}" class="btn btn-secondary btn-footer">Cancel</a>
                            </div>
                        </form>
                    </div>
                </div>
